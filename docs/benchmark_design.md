...
| B2 | 같은 시드 반복 | 기억 없이 같은 시드를 반복했을 때 결과 분산 확인 | B1과 동일 |
| B3 | handoff 학습 | 이전 실패 요약이 다음 런 선택을 바꾸는지 확인 | B1과 동일 |
...
```text
benchmarks/B2_ACT1_PUSH
```

목적은 한 번의 런에서 가능한 한 멀리 진행하는 것이다. 승리보다 먼저 어디서 판단이 무너지는지 본다.
...
```text
benchmarks/B3_HANDOFF_CHAIN
```

B3는 같은 시드를 여러 번 돌리되, 이전 런의 `handoff.json`을 다음 런 입력에 붙인다.
