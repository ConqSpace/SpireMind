...
`--run-log-dir`를 함께 쓰면 `decision_loop`가 최근 기록을 `recent_history`로 함께 보낸다. 여기에는 이전 판단, 행동 결과, 체력 변화, 적 체력 변화가 들어간다. Codex는 이 정보를 참고할 수 있지만, 실제 선택은 항상 현재 `legal_actions` 안에서 해야 한다.
