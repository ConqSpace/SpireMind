...
- `combat_state.json`은 R3부터 `legal_actions` 배열을 포함한다.
- 현재 생성기는 게임 상태를 바꾸지 않고, 손패 기반 후보만 만든다.
- 공격 카드로 판단되면 현재 적 목록의 생존 후보마다 `target_id`가 있는 `play_card` 후보를 만든다.
- 공격이 아닌 카드나 대상 여부가 불확실한 카드는 `target_id: null` 후보로만 만든다.
- 카드 비용, 플레이 가능 여부, 플레이어 에너지, 적 체력이 비어 있으면 `validation_note`에 휴리스틱 검증 필요성을 남긴다.
- `end_turn` 후보는 항상 포함된다.
