...
지도 화면에서는 지금 이동 가능한 다음 노드만 후보 행동으로 만든다.

```json
{
  "action_id": "choose_map_r3_c2",
  "type": "choose_map_node",
  "node_id": "map_r3_c2",
  "row": 3,
  "column": 2,
  "room_type": "Elite",
  "summary": "지도에서 Elite 노드(3, 2)를 선택한다."
}
```

나중에는 같은 행동에 `route_id`나 경로 요약을 붙일 수 있다. 단, 실제 실행은 항상 현재 지도에서 누를 수 있는 다음 노드만 대상으로 한다.
