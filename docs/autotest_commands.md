...
`start_new_run`은 메인 메뉴에서 싱글플레이 새 런을 시작한다. `seed`를 지정하면 표준 런이 아니라 싱글플레이의 커스텀 화면으로 들어가 시드 입력란에 값을 넣은 뒤 시작한다.

고정 시드 커스텀 런 예시:

```json
{
  "id": "cmd-start-seeded-001",
...
- `seed`: 있으면 커스텀 런 화면을 사용한다. 값은 게임과 같은 규칙으로 대문자화하고 `O`는 `0`, `I`는 `1`로 정규화한다.
- `run_mode`: `"custom"`이면 `seed`가 없어도 커스텀 런 화면을 사용한다.
- `custom`: `true`이면 `run_mode: "custom"`과 같다.
- `character_id`: 지정하면 해당 캐릭터 버튼을 선택한다. 없으면 현재 선택되어 있거나 첫 번째로 선택 가능한 캐릭터를 사용한다.
- `timeout_ms`: 전체 새 런 시작 흐름을 기다릴 최대 시간이다.
- `ready_timeout_ms`: 지도, 이벤트, 보상, 전투 상태 파일이 준비되기를 기다릴 최대 시간이다.

안전 조건:

- 메인 메뉴에서 시작해야 한다.
- 커스텀 모드와 시드 기능이 해금되어 있지 않으면 커스텀 버튼이 비활성이라 명령이 진행되지 않는다.
- 요청한 캐릭터가 잠겨 있으면 명령을 실패 처리한다.
