...
| S4 전체 런 시나리오 평가 | 한 판 전체를 비교 가능한 실험으로 만든다 | R6 확장, R7 완료 후 |
...
## 구현 현황 갱신: R5.2 play_card

R5.2에서 `play_card` 실행을 열었다.

- `combat_card_id` 기반으로 손패 카드를 복원한다.
- `target_combat_id` 기반으로 대상 적을 복원한다.
- 실행은 `CardModel.TryManualPlay(Creature?)`로 STS2 내부 액션 큐에 맡긴다.
- 수동 자원 차감, 직접 카드 이동, 직접 효과 호출은 하지 않는다.

다음 구현은 단일 `play_card` 성공을 전제로 한 다중 행동 계획이다. 예를 들어 디버프 카드 사용 후 공격 카드 2장을 이어서 사용하는 흐름을 한 행동씩 상태 갱신을 보며 진행해야 한다.
