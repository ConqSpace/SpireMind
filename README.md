...
처음 사용하는 PC에서는 게임 설치 위치를 먼저 저장한다.

```powershell
node .\scripts\spiremind_setup.js
```

이 명령은 `config/local_setup.local.json`을 만든다. 이 파일에는 STS2 설치 폴더, 실행 파일, `mods` 폴더, 브리지 포트, 기본 시드가 들어간다. 개인 PC 경로이므로 Git에 올리지 않는다.

예시 형식은 [local_setup.example.json](F:/Antigravity/STSAutoplay/config/local_setup.example.json:1)에서 볼 수 있다.

세팅 이후에는 배포와 런타임 점검에서 경로 인자를 생략할 수 있다.
