...
같은 시드를 반복하되, 이전 런에서 생성된 `handoff.json`을 다음 런 판단 요청에 붙입니다.

```powershell
node .\scripts\run_benchmark.js `
  --benchmark-dir .\benchmarks\B3_HANDOFF_CHAIN `
  --decider llm_current `
  --seed seed_0001 `
  --repeat-index 1
...
로컬 HTTP 예시는 [local_http_example.json](./benchmarks/B3_HANDOFF_CHAIN/deciders/local_http_example.json)에 있습니다.
