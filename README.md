...
목표는 자동으로 게임을 클리어하는 봇을 만드는 것이 아닙니다. 이 프로젝트는 LLM이 전투, 보상, 지도, 휴식, 상점, 이벤트 선택을 어떻게 해석하는지 관찰하고, 실패를 다음 시도에서 전략으로 전환하는지 측정합니다.

## 빠른 시작

### 1. 초기 로컬 세팅

처음 사용하는 PC에서는 게임 설치 위치를 저장합니다.
...
배포 위치는 `config/local_setup.local.json`의 `sts2.mods_dir`에서 읽습니다. 필요하면 `.\scripts\deploy_mod.ps1 -ModsDir "<내 STS2 mods 폴더>"`처럼 직접 덮어쓸 수 있습니다.
...
config/                 로컬 설정 예시. *.local.json은 개인 설정이므로 Git에 올리지 않음
...
같은 고정 시드에서 한 런을 가능한 한 멀리 진행합니다. 1막으로 제한하지 않습니다. 종료 조건은 `game_over`, `run_finished`, 또는 최대 판단 횟수입니다.

```powershell
node .\scripts\run_benchmark.js `
  --benchmark-dir .\benchmarks\B1_FULL_RUN `
  --decider llm_current `
  --seed seed_0001 `
  --repeat-index 1 `
  --bridge-url http://127.0.0.1:17832 `
...
같은 시드를 정해진 횟수만큼 반복하되, 이전 런에서 생성된 `handoff.json`을 다음 런 판단 요청에 붙입니다.

```powershell
node .\scripts\run_benchmark.js `
  --benchmark-dir .\benchmarks\B2_HANDOFF_N_RUNS `
  --decider llm_current `
  --seed seed_0001 `
  --repeat-index 1
...
`repeat-index 2`부터는 직전 런의 `handoff.json`이 있으면 자동으로 입력에 포함됩니다.

### B3: handoff 클리어 반복

같은 시드에서 handoff를 계속 이어 붙여 `run_finished`에 도달하거나 정한 중단 한도에 걸릴 때까지 반복합니다. B2가 고정 횟수 실험이라면, B3는 “반복 기회를 충분히 주면 클리어 전략으로 수렴하는가”를 봅니다.

```powershell
node .\scripts\run_benchmark.js `
  --benchmark-dir .\benchmarks\B3_HANDOFF_UNTIL_CLEAR `
  --decider llm_current `
  --seed seed_0001 `
  --repeat-index 1
```
...
| B2 | handoff N회 반복 | 정해진 반복 안에서 이전 실패가 다음 런 행동 규칙으로 바뀌는가? |
| B3 | handoff 클리어 반복 | 반복 기회를 충분히 주면 클리어 전략으로 수렴하는가? |
...
에이전트 실행기는 LLM provider를 직접 알지 않고 `decider` 모듈만 호출합니다.

현재 지원하는 backend:

- `command`: 외부 명령형 판단기
- `app-server`: Codex app-server
- `local-http`: OpenAI 호환 `chat/completions` 엔드포인트

로컬 HTTP 예시는 [local_http_example.json](./benchmarks/B2_HANDOFF_N_RUNS/deciders/local_http_example.json)에 있습니다.
...
- 장기 테스트는 아직 handoff 규칙 이행률 채점이 완성되지 않았습니다.
